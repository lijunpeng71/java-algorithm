...
public class CardsReSortQuestionV2 {
    ...
    private static void process(String lenAndQNum, String cardsStr, String questionStr) {
        ...
        long[] oldCardNums = longCardNums.clone();
        ...
        for (int i = 0; i < qNum; ++i) {
            ...
            int oldIndex = -1;
            for (int j = 0; j < oldCardNums.length; ++j) {
                if (question == oldCardNums[j]) {
                    oldIndex = j;
                    break;
                }
            }
            ...
            int newIndex = binarySearch(longCardNums, 0, longCardNums.length - 1, question);
            System.out.format("%d from %d to %d\n", question, oldIndex + 1, newIndex + 1);
        }
    }
